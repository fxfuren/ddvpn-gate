...
Микросервис создан для **разграничения доступа к различным тарифам на одном домене**. Позволяет пользователям приобретать несколько тарифных планов, при этом обеспечивая безопасность и предотвращая подмену доменов.

**Основные задачи:**
- 📦 **Множественные тарифы**: Поддержка нескольких тарифных планов для разных категорий пользователей
- 🔒 **Защита от подмены**: Предотвращает получение подписки через альтернативные домены
- 🌐 **Единый домен**: Все тарифы доступны на одном домене с разграничением прав
- 👥 **Гибкий контроль**: Разделение доступа по тегам (ADMIN) и принадлежности к squad
